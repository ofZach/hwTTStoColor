...
//pass in array
bool xbeeST::getData(byte * _receivedData,int _timeOut) {    // call when there is serial data
...
if((ofGetElapsedTimeMillis() - startTime) > timeout) return 0;// timed out bad message
  ...
  options = (unsigned char)(Serial->read()); // get the options 1 packet ack returned 2 brodcast
...
for (int i= 0; i < (packetLength - 12); i++) {			// take out other info in packet + checksum
 _receivedData [i] = (unsigned char)(Serial->read());				// get a byte of data
 }
...
bool xbeeST::getDataAuto(int _timeOut) {// call when there is serial data
unsigned int timeout= _timeOut;// bail out time
long startTime;
...
bool xbeeST::sendData(byte* data, int _dataLength,long* _destination,int _localAddress) {
...
bool xbeeST::sendLocalAtCommandWithOptions (char* _ATcommand, char* _AToptions,int _optionsize) {
...
bool xbeeST::sendLocalAtCommand(char* _ATcommand) {
  // set frame ID for call back
  static byte frameID = 0xA1;
  frameID++;                                   // add one to the frame ID each time, okay to overflow
  if (frameID == 0) frameID = 1;               //skip zero because this disables status response packets
...
bool xbeeST::getLocalATCommandData(int _timeOut){
...
bool xbeeST::sendAtCommandWithOptions (long* _destination, char* _ATcommand, char* _AToptions,int _optionsize, int _localAddress ) {
...
bool xbeeST::sendAtCommand(long* _destination, char* _ATcommand, int _localAddress ) {
...
bool xbeeST::getATCommandData(int _timeOut){
  ...
  while (inByte != 0x7E) { // clears out the buffer of junk
    ...
    if((ofGetElapsedTimeMillis() - startTime) > timeout){             //time out for program flow
#ifdef XBEEDEBUG
      Serial->println((ofGetElapsedTimeMillis() - startTime));
...
//DEBUG
 for(int i=0; i < 16; i++){
      ...
      Serial->println((unsigned char)Digitals[i],HEX);
    }
