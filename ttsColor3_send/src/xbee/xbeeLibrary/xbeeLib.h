...
class xbeeST {
	public:
	xbeeST();
	bool sendData(byte* data, int _dataLength,long* _destination,int _localAdress=0xFFFE);
	bool getDataAuto(int _timeOut=500);
	bool getData(byte * _receivedData, int _timeOut=500);

	bool sendAtCommand(long* _destination, char* _ATcommand, int _localAdress=0xFFFE );
	bool sendAtCommandWithOptions (long* _destination, char* _ATcommand, char* _AToptions,int _optionsize, int _localAdress=0xFFFE );
	bool getATCommandData(int _timeOut=500);

	bool sendLocalAtCommandWithOptions (char* _ATcommand, char* _AToptions,int _optionsize);
	bool sendLocalAtCommand(char* _ATcommand);
	bool getLocalATCommandData(int _timeOut= 500);
	...
	const bool* getDigitals() const {return Digitals;}
	...
	bool Digitals[16];  // digital array
