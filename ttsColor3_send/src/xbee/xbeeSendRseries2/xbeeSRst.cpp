...
//pass in array
bool xbeeSRst::getData(byte * _receivedData,int _timeOut) {// call when there is serial data
...
if((ofGetElapsedTimeMillis() - startTime) > timeout) return 0;      // timed out bad message
  ...
  options = (unsigned char)(Serial->read());                    // get the options 1 packet ack returned 2 broadcast
...
bool xbeeSRst::getDataAuto(int _timeOut) {           // call when there is serial data
unsigned int timeout= _timeOut;                         // bail out time
long startTime;
...
bool xbeeSRst::sendData(byte* data, int _dataLength,long* _destination,int _localAddress) {
int dataLength = _dataLength;

static byte frameID = 0xA1;
frameID++; // add one to the frame ID each time, okay to overflow
if (frameID == 0) frameID = 0; //skip zero because this disables status response packets
...
Serial->print(0x01,BYTE); //send option byte about multicasting
